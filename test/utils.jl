...
@info "test 18/62: findpeaks()"
x = rand(100)
x[10] *= 1000
@test 10 in findpeaks(x)

@info "test 19/62: hz2rads()"
@test hz2rads(1) == 2pi

@info "test 20/62: rads2hz()"
@test rads2hz(2pi) == 1
...
@info "test 24/62: generate_window()"
...
@info "test 25/62: generate_sine()"
s = generate_sine(10, 1:100)
@test length(s) == 100

@info "test 26/62: generate_csine()"
s = generate_csine(10, 1:100)
@test length(s) == 100

@info "test 27/62: generate_sinc()"
s = generate_sinc()
@test length(s) == 401

@info "test 28/62: generate_morlet()"
s = generate_morlet(100, 10)
@test length(s) == 201

@info "test 29/62: generate_gaussian()"
s = generate_gaussian(100, 10)
@test length(s) == 201

@info "test 30/62: generate_noise()"
s = generate_noise(100)
@test length(s) == 100

@info "test 31/62: generate_morlet_fwhm()"
