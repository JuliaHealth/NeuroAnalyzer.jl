...
@info "test 18/69: findpeaks()"
x = rand(100)
x[10] *= 1000
@test 10 in findpeaks(x)

@info "test 19/69: hz2rads()"
@test hz2rads(1) == 2pi

@info "test 20/69: rads2hz()"
@test rads2hz(2pi) == 1
...
@info "test 25/69: generate_sine()"
s = generate_sine(10, 1:100)
@test length(s) == 100

@info "test 26/69: generate_csine()"
s = generate_csine(10, 1:100)
@test length(s) == 100

@info "test 27/69: generate_sinc()"
s = generate_sinc()
@test length(s) == 401

@info "test 28/69: generate_morlet()"
s = generate_morlet(100, 10)
@test length(s) == 201

@info "test 29/69: generate_gaussian()"
s = generate_gaussian(100, 10)
@test length(s) == 201

@info "test 30/69: generate_noise()"
s = generate_noise(100)
@test length(s) == 100

@info "test 31/69: generate_morlet_fwhm()"
...
@info "test 63/69: get_channel_bywl()"
@test get_channel_bywl(n, wl=760) == 1:36

@info "test 64/69: size()"
@test size(e10) == (24, 2560, 10)

@info "test 65/69: to_df()"
@test to_df(eeg) isa DataFrame

@info "test 66/69: types()"
@test length(types(e10)) == 24

@info "test 67/69: optode_labels()"
...
@info "test 68/69: datatype()"
@test datatype(e10) == "eeg"

@info "test 69/69: gradient()"
