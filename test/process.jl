...
@info "test 16/49: bpsplit()"
s, bn, bf = bpsplit(e10)
@test length(bn) == 10
@test length(bf) == 10
@test size(s) == (10, 19, 2560, 10)

@info "test 17/49: fconv()"
...
@info "test 40/49: intensity2od()"
n_tmp = intensity2od(n)
@test size(n_tmp.data) == (16, 9015, 1)

@info "test 41/49: od2conc()"
...
@info "test 47/49: pca_reconstruct()"
pc, pcv, pcm, pc_model = pca_decompose(rand(4, 4, 2), n=2)
...
@info "test 48/49: reference_custom()"
e10_tmp = reference_custom(e10)
@test size(e10_tmp.data) == (23, 2560, 10)

@info "test 49/49: ica_reconstruct()"
ic, ic_mw = ica_decompose(rand(10, 1000), n=5)
