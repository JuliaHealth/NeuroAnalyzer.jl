...
@info "Test 8/54: denoise_dwt())"
s = denoise_dwt(rand(100), wt=wavelet(WT.haar))
@test length(s) == 100
e10_tmp = denoise_dwt(e10, wt=wavelet(WT.haar))
@test size(e10_tmp.data) == (24, 2560, 10)

@info "Test 9/54: denoise_wien()"
s = denoise_wien(a1)
@test size(s) == (2, 3, 2)
e10_tmp = denoise_wien(e10)
@test size(e10_tmp.data) == (24, 2560, 10)

@info "Test 10/54: derivative()"
...
@info "Test 16/54: bpsplit()"
s, bn, bf = bpsplit(e10)
@test length(bn) == 13
@test length(bf) == 13
@test size(s) == (13, 23, 2560, 10)

@info "Test 17/54: fconv()"
...
@test size(eeg_tmp.data) == (24, 2560, 10)
eeg_tmp = NeuroAnalyzer.filter(e10, fprototype=:firls, ftype=:lp, cutoff=40)
@test size(eeg_tmp.data) == (24, 2560, 10)
eeg_tmp = NeuroAnalyzer.filter(e10, fprototype=:firls, ftype=:hp, cutoff=1)
@test size(eeg_tmp.data) == (24, 2560, 10)
eeg_tmp = NeuroAnalyzer.filter(e10, fprototype=:firls, ftype=:bs, cutoff=(49, 51))
@test size(eeg_tmp.data) == (24, 2560, 10)
eeg_tmp = NeuroAnalyzer.filter(e10, fprototype=:firls, ftype=:bp, cutoff=(49, 51))
@test size(eeg_tmp.data) == (24, 2560, 10)
...
@info "Test 34/54: reference_plap()"
...
@info "Test 40/54: intensity2od()"
n_tmp = intensity2od(n)
@test size(n_tmp.data) == (16, 9015, 1)

@info "Test 41/54: od2conc()"
...
@info "Test 47/54: pca_reconstruct()"
pc, pcv, pcm, pc_model = pca_decompose(rand(4, 4, 2), n=2)
...
@info "Test 48/54: reference_custom()"
e10_tmp = reference_custom(e10)
@test size(e10_tmp.data) == (23, 2560, 10)

@info "Test 49/54: ica_reconstruct()"
ic, ic_mw = ica_decompose(rand(10, 1000), n=5)
