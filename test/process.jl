...
@test all(
    isapprox.(
        fconv(v1, kernel=v2),
        [
            0.8500000000000002 - 9.868649107779169e-17im,
            1.5999999999999996 + 0.0im,
            2.4999999999999996 + 0.0im,
            3.499999999999999 - 3.61217627448181e-17im,
            2.6999999999999997 - 3.204937810639273e-17im,
        ],
    ),
)
@test all(
    isapprox.(
        fconv(a1, kernel=[0.5, 1.0, 0.5]),
        [
            0.25+0.0im 0.75+0.0im 1.0+0.0im; 0.25+0.0im 0.75+0.0im 1.0+0.0im;;;
            0.25+0.0im 0.75+0.0im 1.0+0.0im; 0.25+0.0im 0.75+0.0im 1.0+0.0im
        ],
    ),
)
...
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:butterworth, ftype=:lp, cutoff=40, order=8
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:butterworth, ftype=:hp, cutoff=1, order=12
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:butterworth, ftype=:bs, cutoff=(49, 51), order=2
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:butterworth, ftype=:bp, cutoff=(49, 51), order=4
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:chebyshev1, ftype=:lp, cutoff=40, order=8, rs=35
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:chebyshev1, ftype=:hp, cutoff=1, order=12, rs=2
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:chebyshev1, ftype=:bs, cutoff=(49, 51), order=2, rs=50
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:chebyshev1, ftype=:bp, cutoff=(49, 51), order=4, rs=50
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:chebyshev2, ftype=:lp, cutoff=40, order=8, rs=35
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:chebyshev2, ftype=:hp, cutoff=1, order=12, rs=2
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:chebyshev2, ftype=:bs, cutoff=(49, 51), order=2, rs=50
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:chebyshev2, ftype=:bp, cutoff=(49, 51), order=4, rs=50
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:elliptic, ftype=:lp, cutoff=40, order=8, rs=35
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:elliptic, ftype=:hp, cutoff=1, order=12, rs=2
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:elliptic, ftype=:bs, cutoff=(49, 51), order=2, rs=50
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:elliptic, ftype=:bp, cutoff=(49, 51), order=4, rs=50
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:fir, ftype=:lp, cutoff=40, order=91
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:fir, ftype=:hp, cutoff=1, order=91
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:fir, ftype=:bs, cutoff=(49, 51), order=91
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:fir, ftype=:bp, cutoff=(49, 51), order=91
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:firls, ftype=:lp, cutoff=40, order=32, bw=2
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:firls, ftype=:hp, cutoff=10, order=32, bw=2
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:firls, ftype=:bs, cutoff=(49, 51), order=32, bw=2
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:firls, ftype=:bp, cutoff=(49, 51), order=32, bw=2
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:remez, ftype=:lp, cutoff=40, order=32, bw=10
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:remez, ftype=:hp, cutoff=10, order=32, bw=0.5
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:remez, ftype=:bs, cutoff=(49, 51), order=32, bw=0.1
)
@test size(eeg_tmp) == size(e10)
eeg_tmp = NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:remez, ftype=:bp, cutoff=(49, 51), order=32, bw=0.5
)
@test size(eeg_tmp) == size(e10)
...
@test normalize_zscore(m1, bych=false) == [
    -1.3363062095621219 -0.8017837257372732 -0.2672612419124244;
...
@test normalize_minmax(m1, bych=false) ==
...
@test normalize_log(m1, bych=true) == [
    1.0986122886681098 1.3862943611198906 1.6094379124341003;
    2.1972245773362196 2.302585092994046 2.3978952727983707
]
@test normalize_log(m1, bych=false) == [
    1.0986122886681098 1.3862943611198906 1.6094379124341003;
    1.791759469228055 1.9459101490553132 2.0794415416798357
]
@test normalize_gauss(v1) == [
    -0.8047189562170504, -0.3465735902799727, 0.0, 0.3465735902799726, 0.8047189562170504
]
@test normalize_gauss(m1, bych=true) == [
    -0.5493061443340549 0.0 0.5493061443340549;
    -0.5493061443340549 0.0 0.5493061443340549
]
@test normalize_gauss(m1, bych=false) == [
    -0.8958797346140276 -0.45814536593707755 -0.1438410362258905;
    0.1438410362258904 0.45814536593707755 0.8958797346140273
]
@test normalize_log10(v1) == [
    0.47712125471966244,
    0.6020599913279624,
    0.6989700043360189,
    0.7781512503836436,
...
@test normalize_log10(m1, bych=true) == [
    0.47712125471966244 0.6020599913279624 0.6989700043360189;
    0.9542425094393249 1.0 1.0413926851582251
]
@test normalize_log10(m1, bych=false) == [
    0.47712125471966244 0.6020599913279624 0.6989700043360189;
    0.7781512503836436 0.8450980400142568 0.9030899869919435
]
@test normalize_neglog(v1) == [
    -0.0, -0.6931471805599453, -1.0986122886681098, -1.3862943611198906, -1.6094379124341003
]
@test normalize_neglog(m1, bych=true) == [
    -0.0 -0.6931471805599453 -1.0986122886681098;
    -1.3862943611198906 -1.6094379124341003 -1.791759469228055
]
@test normalize_neglog(m1, bych=false) == [
    -0.0 -0.6931471805599453 -1.0986122886681098;
    -1.3862943611198906 -1.6094379124341003 -1.791759469228055
]
@test normalize_neglog10(v1) == [
    -0.0,
    -0.3010299956639812,
    -0.47712125471966244,
    -0.6020599913279624,
...
@test normalize_invroot(v1) ==
    [1.0, 0.7071067811865475, 0.5773502691896258, 0.5, 0.4472135954999579]
@test normalize_invroot(m1, bych=true) ==
    [1.0 0.7071067811865475 0.5773502691896258; 0.5 0.4472135954999579 0.4082482904638631]
@test normalize_invroot(m1, bych=false) ==
    [1.0 0.7071067811865475 0.5773502691896258; 0.5 0.4472135954999579 0.4082482904638631]
@test normalize_softmax(v1) == [
...
@test normalize_softmax(m1, bych=true) == [
    0.0042697785452821095 0.011606461431184656 0.03154963320110001;
    0.08576079462509834 0.23312200962361299 0.6336913225737218
]
@test normalize_softmax(m1, bych=false) == [
    0.0042697785452821095 0.011606461431184656 0.03154963320110001;
    0.08576079462509834 0.23312200962361299 0.6336913225737218
]
@test normalize_sigmoid(v1) == [
    0.7310585786300049,
    0.8807970779778823,
    0.9525741268224334,
    0.9820137900379085,
...
@test normalize_sigmoid(m1, bych=true) == [
    0.7310585786300049 0.8807970779778823 0.9525741268224334;
    0.9820137900379085 0.9933071490757153 0.9975273768433653
]
@test normalize_sigmoid(m1, bych=false) == [
    0.7310585786300049 0.8807970779778823 0.9525741268224334;
    0.9820137900379085 0.9933071490757153 0.9975273768433653
]
@test normalize_mad(v1) == [
    -0.9098742077378683, -0.45493710386893416, 0.0, 0.45493710386893416, 0.9098742077378683
]
@test normalize_mad(m1, bych=true) == [
    -0.45493710386893416 0.0 0.45493710386893416;
    -0.45493710386893416 0.0 0.45493710386893416
]
@test normalize_mad(m1, bych=false) == [
    -1.1241495836601363 -0.6744897501960818 -0.22482991673202726;
    0.22482991673202726 0.6744897501960818 1.1241495836601363
]
...
@test normalize_fisher(v1) ==
    [-18.36840028483855, -0.5493061443340549, 0.0, 0.5493061443340549, 18.36840028483855]
@test normalize_fisher(m1, bych=true) == [
    -18.36840028483855 -0.6931471805599453 -0.20273255405408214;
    0.2027325540540821 0.6931471805599454 18.36840028483855
]
@test normalize_fisher(m1, bych=false) == [
    -18.36840028483855 -0.6931471805599453 -0.20273255405408214;
    0.2027325540540821 0.6931471805599454 18.36840028483855
]
...
@info "Test: emd()"
@test size(emd(e10.data[1, :, 1], e10.epoch_time, epsilon=0.3), 1) in [8, 9]
@test size(emd(e10, ch="Fp1", ep=1, epsilon=0.3), 1) in [8, 9]
