...
@info "test 16/50: bpsplit()"
s, bn, bf = bpsplit(e10)
@test length(bn) == 10
@test length(bf) == 10
@test size(s) == (10, 19, 2560, 10)

@info "test 17/50: fconv()"
...
@info "test 40/50: intensity2od()"
n_tmp = intensity2od(n)
@test size(n_tmp.data) == (16, 9015, 1)

@info "test 41/50: od2conc()"
...
@info "test 47/50: pca_reconstruct()"
pc, pcv, pcm, pc_model = pca_decompose(rand(4, 4, 2), n=2)
...
@info "test 48/50: reference_custom()"
e10_tmp = reference_custom(e10)
@test size(e10_tmp.data) == (23, 2560, 10)

@info "test 49/50: ica_reconstruct()"
ic, ic_mw = ica_decompose(rand(10, 1000), n=5)
