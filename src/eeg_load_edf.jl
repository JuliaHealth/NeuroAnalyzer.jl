...
function eeg_load_edf(in_file, read_annotations=true, header_only=false, clean_labels=false)
    ...
    eeg_file_header = Dict(:version => version, :eeg_filetype => eeg_filetype, :patient => patient, :recording => recording, :recording_date => recording_date, :recording_time => recording_time, :data_records => data_records, :data_records_duration => data_records_duration, :channels_no => channels_no, :reference_type => "", :reference_channel => [], :xlocs => [], :ylocs => [])
