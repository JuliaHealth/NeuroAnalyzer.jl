...
function locs_generate(locs::DataFrame)::DataFrame
    ...
    let (xi, yi, zi) = sph2cart(1.0, 0, 90)
        x[lab .== "cz"] .= xi;  y[lab .== "cz"] .= yi;  z[lab .== "cz"] .= zi
    end
    let (xi, yi, zi) = sph2cart(1.0, 0, 67.5)
        x[lab .== "c2"] .= xi;  y[lab .== "c2"] .= yi;  z[lab .== "c2"] .= zi
    end
    let (xi, yi, zi) = sph2cart(1.0, 0, 45)
        x[lab .== "c4"] .= xi;  y[lab .== "c4"] .= yi;  z[lab .== "c4"] .= zi
    end
    let (xi, yi, zi) = sph2cart(1.0, 0, 22.5)
        x[lab .== "c6"] .= xi;  y[lab .== "c6"] .= yi;  z[lab .== "c6"] .= zi
    end
    let (xi, yi, zi) = sph2cart(1.0, 0, 0)
        mask = lab .== "t4" .|| lab .== "t8"
        x[mask] .= xi;  y[mask] .= yi;  z[mask] .= zi
    end
    let (xi, yi, zi) = sph2cart(1.0, -22.5, 0)
        x[lab .== "t10"] .= xi;  y[lab .== "t10"] .= yi;  z[lab .== "t10"] .= zi
    end
    let (xi, yi, zi) = sph2cart(1.0, 0, 112.5)
        x[lab .== "c1"] .= xi;  y[lab .== "c1"] .= yi;  z[lab .== "c1"] .= zi
    end
    let (xi, yi, zi) = sph2cart(1.0, 0, 135)
        x[lab .== "c3"] .= xi;  y[lab .== "c3"] .= yi;  z[lab .== "c3"] .= zi
    end
    let (xi, yi, zi) = sph2cart(1.0, 0, 157.5)
        x[lab .== "c5"] .= xi;  y[lab .== "c5"] .= yi;  z[lab .== "c5"] .= zi
    end
    ...
    let (xi, yi, zi) = sph2cart(1.0, -22.5, 180)
        x[lab .== "t9"] .= xi;  y[lab .== "t9"] .= yi;  z[lab .== "t9"] .= zi
    end
    ...
    let r = cosd(45), off = cosd(45)
        let (xi, yi, zi) = sph2cart(1.0, 90, 45)
            x[lab .== "fz"] .= xi;  y[lab .== "fz"] .= yi;  z[lab .== "fz"] .= zi
        end
        ...
        let (xi, yi, zi) = sph2cart(r, -22.5, 0)
            x[lab .== "f10"] .= xi;  y[lab .== "f10"] .= off .+ yi;  z[lab .== "f10"] .= zi
        end
        ...
        let (xi, yi, zi) = sph2cart(r, -22.5, 180)
            x[lab .== "f9"] .= xi;  y[lab .== "f9"] .= off .+ yi;  z[lab .== "f9"] .= zi
        end
    end
    ...
    let r = cosd(67.5), off = cosd(67.5)
        let (xi, yi, zi) = sph2cart(1.0, 90, 67.5)
            x[lab .== "afz"] .= xi;  y[lab .== "afz"] .= yi;  z[lab .== "afz"] .= zi
        end
        for (lbl, phi) in [("af2", 67.5), ("af4", 45), ("af6", 22.5),
    ...
    let r = cosd(22.5), off = -cosd(67.5)
        ...
        let (xi, yi, zi) = sph2cart(r, -22.5, 0)
            x[lab .== "tp10"] .= xi;  y[lab .== "tp10"] .= off .+ yi;  z[lab .== "tp10"] .= zi
        end
        let (xi, yi, zi) = sph2cart(r, -22.5, 180)
            x[lab .== "tp9"] .= xi;  y[lab .== "tp9"] .= off .+ yi;  z[lab .== "tp9"] .= zi
        end
    ...
    let r = cosd(45), off = -cosd(45)
        let (xi, yi, zi) = sph2cart(1.0, 270, 45)
            x[lab .== "pz"] .= xi;  y[lab .== "pz"] .= yi;  z[lab .== "pz"] .= zi
        end
        ...
        let (xi, yi, zi) = sph2cart(r, -22.5, 0)
            x[lab .== "p10"] .= xi;  y[lab .== "p10"] .= off .+ yi;  z[lab .== "p10"] .= zi
        end
        ...
        let (xi, yi, zi) = sph2cart(r, -22.5, 180)
            x[lab .== "p9"] .= xi;  y[lab .== "p9"] .= off .+ yi;  z[lab .== "p9"] .= zi
        end
    end
    ...
    let r = cosd(67.5), off = -cosd(67.5)
        let (xi, yi, zi) = sph2cart(1.0, 270, 22.5)
            x[lab .== "poz"] .= xi;  y[lab .== "poz"] .= yi;  z[lab .== "poz"] .= zi
        end
        for (lbl, phi) in [("po2", 67.5), ("po4", 45), ("po6", 22.5), ("po8", 0),
    ...
    for (lbl, az, phi) in [("oz", 270, 0), ("o2", 292.5, 0), ("o1", 247.5, 0)]
        let (xi, yi, zi) = sph2cart(1.0, az, phi)
            x[lab .== lbl] .= xi;  y[lab .== lbl] .= yi;  z[lab .== lbl] .= zi
        end
    end
    ...
    x[lab .== "a1"] .= -0.92;  y[lab .== "a1"] .= -0.23;  z[lab .== "a1"] .= -0.55
    x[lab .== "a2"] .=  0.92;  y[lab .== "a2"] .= -0.23;  z[lab .== "a2"] .= -0.55
    x[lab .== "m1"] .= -0.94;  y[lab .== "m1"] .= -0.10;  z[lab .== "m1"] .= -0.30
    x[lab .== "m2"] .=  0.94;  y[lab .== "m2"] .= -0.10;  z[lab .== "m2"] .= -0.30

    # EMG electrodes (chin/neck placement)
    x[lab .== "emg1"] .= -0.7;  y[lab .== "emg1"] .= 0.7;  z[lab .== "emg1"] .= -1.1
    x[lab .== "emg2"] .=  0.7;  y[lab .== "emg2"] .= 0.7;  z[lab .== "emg2"] .= -1.1

    # EOG electrodes: vertical (VEOG) and horizontal (HEOG) eye movements
    for lbl in ("eog1", "veog1")
        x[lab .== lbl] .= -0.87;  y[lab .== lbl] .= 0.51;  z[lab .== lbl] .= -0.37
    end
    for lbl in ("eog2", "veog2", "veog")
        x[lab .== lbl] .=  0.87;  y[lab .== lbl] .= 0.51;  z[lab .== lbl] .= -0.37
    end
    for lbl in ("heog1", "leog")
        x[lab .== lbl] .= -0.64;  y[lab .== lbl] .= 0.77;  z[lab .== lbl] .= -0.04
    end
    ...
    if !isempty(no_match)
        _warn("Location$(_pl(length(no_match))): $(uppercase.(no_match)) could not be generated.")
    end
...
function locs_generate!(locs::DataFrame)::Nothing
    ...
    for col in (:loc_radius, :loc_theta, :loc_x, :loc_y, :loc_z,
