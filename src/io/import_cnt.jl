...
function import_cnt(file_name::String; data_format::Symbol=:i32, detect_type::Bool=true)
    ...
    type = _fread(fid, 1, :c)
    id = _v2s(_fread(fid, 20, :c))
    oper = _v2s(_fread(fid, 20, :c))
    doctor = _v2s(_fread(fid, 20, :c))
    referral = _v2s(_fread(fid, 20, :c))
    hospital = _v2s(_fread(fid, 20, :c))
    patient = _v2s(_fread(fid, 20, :c))
    age = _fread(fid, 1, :i16)
    sex = _fread(fid, 1, :c)
    hand = _fread(fid, 1, :c)
    med = _v2s(_fread(fid, 20 , :c))
    category = _v2s(_fread(fid, 20 , :c))
    state = _v2s(_fread(fid, 20 , :c))
    label = _v2s(_fread(fid, 20 , :c))
    recording_date = _v2s(_fread(fid, 10 , :c))
    recording_time = _v2s(_fread(fid, 12 , :c))
    ...
    for idx in 1:ch_n
        push!(clabels, _v2s(_fread(fid, 10, :c)))
