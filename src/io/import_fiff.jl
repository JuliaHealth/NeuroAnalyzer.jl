...
function import_fiff(file_name::String; detect_type::Bool=true)
    ...
    @inbounds for block_idx in eachindex(buf)
        ...
        elseif tag_type in [210]
            # aspect
            d = _find_fiff_aspect(Int64(ntoh.(reinterpret(Int32, buf_tmp)[1])))
        elseif tag_type in [264]
            # sss_job
            d = _find_fiff_sss_job(Int64(ntoh.(reinterpret(Int32, buf_tmp)[1])))
        ...
        elseif tag_type in [115]
            # role
            d = @views Int64(ntoh.(reinterpret(Int32, buf_tmp)[1]))
            d = d == 1 ? "prev_file" : "next_file"
        elseif tag_type in [234]
            # dig_string
            kind = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[1:4])[1]))
            ident = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[5:8])[1]))
            np = @views Float64(ntoh.(reinterpret(Int32, buf_tmp[9:12])[1]))
            rr = Float64[]
            [push!(rr, @views Float64(ntoh.(reinterpret(Float32, buf_tmp[idx:(idx + 3)])[1]))) for idx in 13:4:length(buf_tmp)]
            d = (kind, ident, np, rr)
        elseif tag_type in [255]
            # ch_pos
            coil_type = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[1:4])[1]))
            r0_1 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[5:8])[1]))
            r0_2 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[9:12])[1]))
            r0_3 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[13:16])[1]))
            ex_1 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[17:20])[1]))
            ex_2 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[21:24])[1]))
            ex_3 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[25:28])[1]))
            ey_1 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[29:32])[1]))
            ey_2 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[33:36])[1]))
            ey_3 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[37:40])[1]))
            ez_1 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[41:44])[1]))
            ez_2 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[45:48])[1]))
            ez_3 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[59:52])[1]))
        ...
        elseif tag_type in [242]
            # hpi_mask
            d = @views ntoh.(reinterpret(UInt32, buf_tmp)[1])
        elseif tag_type in [222]
            # coord_trans
            from = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[1:4])[1]))
            to = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[5:8])[1]))
            rot = zeros(3, 3)
            for idx1 in 1:3
                d = Float64[]
                [push!(d, @views Float64(ntoh.(reinterpret(Int16, buf_tmp[(8 + idx2):(8 + idx2 + 3)])[1]))) for idx2 in 1:4:9]
                rot[idx1, :] = d
            end
            move = Float64[]
            [push!(move, @views Float64(ntoh.(reinterpret(Int16, buf_tmp[(44 + idx):(44 + idx + 3)])[1]))) for idx in 1:4:9]
            invrot = zeros(3, 3)
            for idx1 in 1:3
                d = Float64[]
                [push!(d, @views Float64(ntoh.(reinterpret(Int16, buf_tmp[(56 + idx2):(56 + idx2 + 3)])[1]))) for idx2 in 1:4:9]
                invrot[idx1, :] = d
            end
            invmove = Float64[]
            [push!(invmove, @views Float64(ntoh.(reinterpret(Int16, buf_tmp[(92 + idx):(92 + idx + 3)])[1]))) for idx in 1:4:9]
        ...
        elseif tag_type in [300]
            ...
            if df == "dau_pack16" || df == "int16"
                [push!(d, @views Float64(ntoh.(reinterpret(Int16, buf_tmp[idx:(idx + 1)])[1]))) for idx in 1:2:length(buf_tmp)]
            elseif df == "int32"
                [push!(d, @views Float64(ntoh.(reinterpret(Int32, buf_tmp[idx:(idx + 3)])[1]))) for idx in 1:4:length(buf_tmp)]
            elseif df == "float"
                [push!(d, @views Float64(ntoh.(reinterpret(Float32, buf_tmp[idx:(idx + 3)])[1]))) for idx in 1:4:length(buf_tmp)]
            else
        ...
        elseif tag_type in [213]
            ...
            kind = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[1:4])[1]))
            ident = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[5:8])[1]))
            r_1 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[9:12])[1]))
            r_2 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[13:16])[1]))
            r_3 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[17:20])[1]))
            d = (kind, ident, r_1, r_2, r_3)
        elseif tag_type in [252]
            # ch_type
            d =  @views _find_fiff_chtype(Int64(ntoh.(reinterpret(Int32, buf_tmp)[1])))
        elseif tag_type in [256]
            # ch_unit
            d = _find_fiff_unit(Int64(ntoh.(reinterpret(Int32, buf_tmp)[1])))
        elseif tag_type in [3411]
            # proj_item
            d = _find_fiff_proj_item(Int64(ntoh.(reinterpret(Int32, buf_tmp)[1])))
        elseif tag_type in [3416]
            # proj_item
            d = _find_fiff_proj_by(Int64(ntoh.(reinterpret(Int32, buf_tmp)[1])))
        elseif tag_type in [280]
            # gantry_type
            d = _find_fiff_gantry_type(Int64(ntoh.(reinterpret(Int32, buf_tmp)[1])))
        elseif tag_type in [203]
            # ch_info
            scan_no = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[1:4])[1]))
            log_no = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[5:8])[1]))
            kind = @views _find_fiff_chtype(Int64(ntoh.(reinterpret(Int32, buf_tmp[9:12])[1])))
            range = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[13:16])[1]))
            cal = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[17:20])[1]))
            coil_type = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[21:24])[1]))
            r0_1 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[25:28])[1]))
            r0_2 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[29:32])[1]))
            r0_3 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[33:36])[1]))
            ex_1 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[37:40])[1]))
            ex_2 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[41:44])[1]))
            ex_3 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[45:48])[1]))
            ey_1 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[49:52])[1]))
            ey_2 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[53:56])[1]))
            ey_3 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[57:60])[1]))
            ez_1 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[61:64])[1]))
            ez_2 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[65:68])[1]))
            ez_3 = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[69:72])[1]))
            unit = @views _find_fiff_unit(Int64(ntoh.(reinterpret(Int32, buf_tmp[73:76])[1])))
            unit_mul = @views _find_fiff_mul(Int64(ntoh.(reinterpret(Int32, buf_tmp[77:80])[1])))
        ...
        elseif tag_type in [282]
            # int8
            d = ntoh.(reinterpret(Int8, buf_tmp[1:4])[1])
        elseif tag_type in [101, 102, 104, 105, 106, 117, 200, 202, 204, 207, 208, 209, 211, 216, 217, 221, 228, 230, 231, 245, 250, 251, 257, 263, 266, 267, 268, 277, 278, 301, 303, 400, 500, 701, 702, 703, 2004, 2010, 2012, 2014, 2023, 2032, 3013, 3104, 3414]
            # int32
            d = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[1:4])[1]))
            if tag_type == 204
                d = unix2datetime(d)
            end
        elseif tag_type in [220, 232, 225, 246, 247, 269, 304, 305, 600, 601, 603, 3413]
            # int32*
            d = Int64[]
            [push!(d, @views Int64(ntoh.(reinterpret(Int32, buf_tmp[idx:(idx + 3)])[1]))) for idx in 1:4:length(buf_tmp)]
        elseif tag_type in [276]
            # double
            d = @views Float64(ntoh.(reinterpret(Float64, buf_tmp[1:4])[1]))
        elseif tag_type in [201, 218, 219, 223, 229, 235, 236, 240, 241, 243, 244, 253, 254, 272, 279, 407, 408, 2005, 2009, 2011, 2013, 2015, 2016, 2018, 2019, 2024, 2040, 3109, 3113, 3405, 3412]
            # float
            d = @views Float64(ntoh.(reinterpret(Float32, buf_tmp[1:4])[1]))
        ...
        elseif tag_type in [100, 103, 109, 110, 116, 120]
            # id_t
            fiff_v_major = @views Int64(ntoh.(reinterpret(Int16, buf_tmp[1:2])[1]))
            fiff_v_minor = @views Int64(ntoh.(reinterpret(Int16, buf_tmp[3:4])[1]))
            mach_id1 = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[5:8])[1]))
            mach_id2 = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[9:12])[1]))
            time_sec = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[13:16])[1]))
            id_creation_date = unix2datetime(time_sec)
            time_usec = @views Int64(ntoh.(reinterpret(Int32, buf_tmp[17:20])[1]))
    ...
    raw_data[:raw_data] = @views reshape(raw_data[:raw_data], meas_info[:nchan], :, 1)
    ...
    clabels = repeat([""], ch_n)
    @inbounds for (k, v) in meas_info[:ch_info]
        ch = v[1]
        ch_type[ch] = v[3]
        range = v[4]
        cal = v[5]
        ...
        @views raw_data[:raw_data][ch, :] .*= (range * cal * unit_mul)
    end

    # convert data to standard units
    @inbounds for ch_idx in 1:ch_n
        if units[ch_idx] == "T"
            @views raw_data[:raw_data][ch_idx, :, :] .*= 10^15
            units[ch_idx] = "fT"
        end
        if units[ch_idx] == "T/m"
            @views raw_data[:raw_data][ch_idx, :, :] .*= (10^15 / 100)
            units[ch_idx] = "fT/cm"
        end
        if units[ch_idx] == "V" && ch_type[ch_idx] in ["eeg", "emg", "eog", "ref"]
            @views raw_data[:raw_data][ch_idx, :, :] .*= 10^6
            units[ch_idx] = "μV"
        end 
    end
