...
_find_fiff_block(t::String)::Int64 = fiff_blocks[:id][findfirst(
    isequal(t), fiff_blocks[:block]
)]
_find_fiff_block(n::Int64)::String = fiff_blocks[:block][findfirst(
    isequal(n), fiff_blocks[:id]
)]
_find_fiff_dt(n::Int64)::String = fiff_data_type[:name][findfirst(
    isequal(n & 0x00000FFF), fiff_data_type[:id]
)]
_find_fiff_unit(n::Int64)::String = fiff_units[:unit][findfirst(
    isequal(n), fiff_units[:id]
)]
_find_fiff_mul(n::Int64)::Int64 = fiff_multipliers[:id][findfirst(
    isequal(n), fiff_multipliers[:id]
)]
_find_fiff_chtype(n::Int64)::String = fiff_channel_type[:channel_type][findfirst(
    isequal(n), fiff_channel_type[:id]
)]
_find_fiff_gantry_type(n::Int64)::String = fiff_gantry_type[:gantry_type][findfirst(
    isequal(n), fiff_gantry_type[:id]
)]
_find_fiff_dacq_system(n::Int64)::String = fiff_dacq_system[:dacq_system][findfirst(
    isequal(n), fiff_dacq_system[:id]
)]
_find_fiff_proj_item(n::Int64)::String = fiff_proj_item[:proj_item][findfirst(
    isequal(n), fiff_proj_item[:id]
)]
_find_fiff_proj_by(n::Int64)::String = fiff_proj_by[:proj_by][findfirst(
    isequal(n), fiff_proj_by[:id]
)]
_find_fiff_coiltype(n::Int64)::String = fiff_coil_type[:coil_type][findfirst(
    isequal(n), fiff_coil_type[:id]
)]
_find_fiff_aspect(n::Int64)::String = fiff_aspect[:aspect][findfirst(
    isequal(n), fiff_aspect[:id]
)]
_find_fiff_sss_job(n::Int64)::String = fiff_sss_job[:sss_job][findfirst(
    isequal(n), fiff_sss_job[:id]
)]
...
fiff_tags = Dict(
    ...
    :tag=>[
        ...
        "mri_pixel_scale",
        "mri_pixel_data",
        "mri_pixel_overlay_encoding",
        "mri_pixel_overlay_data",
        "mri_bounding_box",
        "mri_width",
        "mri_width_m",
        "mri_height",
        ...
        "mri_thickness",
        "mri_scene_aim",
        "mri_calibration_scale",
        "mri_calibration_offset",
        "mri_orig_source_path",
        "mri_orig_source_format",
        "mri_orig_pixel_encoding",
        "mri_orig_pixel_data_offset",
        "mri_time",
        "mri_voxel_data",
        "mri_voxel_encoding",
        "voxel_nchannels",
        "mri_diffusion_weight",
        "mri_diffusion_param",
        "mri_mrilab_setup",
        "mri_seg_region_id",
        "conductor_model_kind",
        "sphere_origin",
        "modelsphere_coord_frame",
        "sphere_layers",
        "bem_surf_id",
        "bem_surf_name",
        "bem_surf_nnode",
        ...
        "xfit_lead_products",
        "xfit_map_products",
        "xfit_grad_map_products",
        "xfit_vol_integration",
        "xfit_integration_radius",
        "xfit_conductor_model_name",
        "xfit_conductor_model_trans_name",
        "xfit_cont_surf_type",
        "proj_item_kind",
        "proj_item_time",
        "proj_item_ign_chs",
...
fiff_dacq_system = Dict(
    :id=>[0, 1, 2, 3, 4], :dacq_system=>["dau", "vxi", "rpu", "orion", "triux"]
)
...
fiff_proj_item = Dict(
    :id=>[0, 1, 2, 3, 4, 5, 10],
    :proj_item=>[
        "none", "field", "dip_fix", "dip_rot", "homog_grad", "homog_field", "eeg_avref"
    ],
)
...
function _fiff_matrix(fb::Int64, buf::Vector{UInt8})::Union{Vector{Float64},Matrix{Float64}}
    ...
    if fs_mask == 0x00000000 # scalar
        ...
        elseif df == "old_pack"
            ...
            _warn(
                "scalar of $df is not implemented yet; if you have such a file, please send it to adam.wysokinski@neuroanalyzer.org",
            )
        end
    elseif fs_mask == 0x40000000 # matrix
        ...
        if mc_mask == 0x00000000 # dense
            ...
            [
                push!(dim, _i32i32(dims[dim_idx:(dim_idx + 3)])) for
                dim_idx in 1:4:length(dims)
            ]
            reverse!(dim)
            ...
            elseif df == "old_pack"
                ...
                _warn(
                    "matrix of $df is not implemented yet; if you have such a file, please send it to adam.wysokinski@neuroanalyzer.org",
                )
            end
        ...
        elseif mc_mask == 0x00100000 # sparse, column-compressed
            ...
            if df == "float"
                ...
                _warn(
                    "sparse, column-compressed of $df is not implemented yet; if you have such a file, please send it to adam.wysokinski@neuroanalyzer.org",
                )
            end
            d = zeros(dim)
            col = 1
        ...
        elseif mc_mask == 0x00200000 # sparse, row-compressed
            _warn(
                "sparse, row-compressed is not implemented yet; if you have such a file, please send it to adam.wysokinski@neuroanalyzer.org",
            )
        end
    end
    return d
...
function _read_fiff_tag(fid::IOStream)::Tuple{Int32,Int32,Int32,Any,Int32}
...
function _pack_fiff_blocks(
    fiff_object::Vector{Any}, block::String, fields::Vector{String}
)::Dict
    ...
    for f in fields
        ...
        elseif length(tmp) > 1
            [
                push!(
                    d,
                    Symbol(f * "_$(lpad(n, length(string(length(tmp))), '0'))")=>tmp[n][4],
