function _get_t(obj::NeuroAnalyzer.NEURO)::Tuple{Vector{Float64},Vector{Float64}}
    fs = sr(obj)
    time_pts = round.(
        collect(0:(1 / fs):(size(obj.data, 2) * size(obj.data, 3) / fs))[1:(end - 1)];
        digits=4,
    )
    if length(obj.epoch_time) > 0
        epoch_time = round.(
            (collect(0:(1 / fs):(size(obj.data, 2) / fs)) .+ obj.epoch_time[1])[1:(end - 1)];
            digits=4,
        )
    else
        epoch_time = round.(
            (collect(0:(1 / fs):(size(obj.data, 2) / fs)))[1:(end - 1)]; digits=4
        )
    end
...
function _s2epoch(
    obj::NeuroAnalyzer.NEURO, from::Int64, to::Int64
