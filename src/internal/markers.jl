function _delete_markers(
    markers::DataFrame, segment::Tuple{Real,Real}, fs::Int64
)::DataFrame
    for marker_idx in DataFrames.nrow(markers):-1:1
        !(round(Int64, fs * markers[marker_idx, :start]) in segment[1]:segment[2]) &&
            deleteat!(markers, marker_idx)
    end
    return markers
end
...
function _shift_markers(m::DataFrame, pos::Real, offset::Real, fs::Int64)::DataFrame
    markers = deepcopy(m)
    for marker_idx in 1:DataFrames.nrow(markers)
        round(Int64, fs * markers[marker_idx, :start]) > pos &&
            (markers[marker_idx, :start] -= (offset / fs))
    end
    return markers
end
...
function _get_epoch_markers(obj::NeuroAnalyzer.NEURO)::Vector{Float64}
    return round.(
        s2t.(collect(1:epoch_len(obj):(epoch_len(obj) * nepochs(obj))), sr(obj)); digits=4
    )
end
