function _draw_head(
    ...
    if head_details
        nose = Plots.Shape([
            (-0.2, maxy - 0.015), (0, maxy + 0.08), (0.2, maxy - 0.015), (-0.01, maxy)
        ])
        ear_r = Plots.Shape([
            (maxx, -0.05),
            (maxx - 0.005, 0.09),
    ...
    if head_labels
        p = Plots.plot!(;
            annotations=(
                0, 1.05, Plots.text("IN"; pointsize=4, halign=:center, valign=:center)
            ),
        )
        p = Plots.plot!(;
            annotations=(
                0, -1.05, Plots.text("NAS"; pointsize=4, halign=:center, valign=:center)
            ),
        )
        p = Plots.plot!(;
            annotations=(
                -1.05,
                0,
                Plots.text("LPA"; pointsize=4, halign=:center, valign=:center, rotation=90),
            ),
        )
        p = Plots.plot!(;
            annotations=(
                ...
                0,
                Plots.text(
                    "RPA"; pointsize=4, halign=:center, valign=:center, rotation=-90
                ),
            ),
        )
    end
