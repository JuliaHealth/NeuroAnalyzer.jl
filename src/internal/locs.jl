function _sph_distance_sph(
    r1::Real, theta1::Real, phi1::Real, r2::Real, theta2::Real, phi2::Real
)
    d = sqrt(
...
function _check_ch_locs(
    ch::Union{Int64,Vector{Int64}}, objl::Vector{String}, locsl::Vector{String}
)::Nothing
    for idx in ch
...
function _find_bylabel(
    ...
    if isa(l, String)
        ...
        l_idx = Vector{Int64}()
        for idx in l
            lowercase(idx) in lowercase.(locs[!, :label]) && push!(
                l_idx, findfirst(isequal.(lowercase(idx), lowercase.(locs[!, :label])))
            )
        end
        return l_idx
    end
