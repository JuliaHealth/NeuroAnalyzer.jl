...
function pcacomp(
    ...
    n_tmp = n
    if zstd
        pc_tmp = @views MultivariateStats.fit(
            PCA, Matrix(m'), maxoutdim=n, pratio=1, mean=0
        )
    else
...
function biplot(
    ...
    if n >= 2
        ...
        GLMakie.scatter!(
            ax, pca.pc[:, "PC1"], pca.pc[:, "PC2"]; markersize=10, colormap=:darktest
        )
...
function screeplot(
    ...
    for idx in 1:n
        GLMakie.barplot!(
            ax1, idx, pca.pcv[idx]; color=cmap[idx], colorrange=1:n, colormap=:darktest
        )
    end
