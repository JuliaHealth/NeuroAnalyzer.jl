...
function header(obj::NeuroAnalyzer.NEURO)::Nothing
    ...
    if length(obj.header.subject[:id]) > 0
        ...
        println(
            "                Subject: $(obj.header.subject[:first_name] * " " * obj.header.subject[:last_name])",
        )
    end
...
function channel_info(
    obj::NeuroAnalyzer.NEURO; ch::Int64, pr::Bool=true
...
function channel_cluster(obj::NeuroAnalyzer.NEURO; cluster::Symbol)::Vector{Int64}
    ...
    cluster === :o &&
        (cluster = ["O1", "O2", "POz", "PO3", "PO4", "PO7", "PO8", "PO9", "PO10"])
