...
- `bm::Vector{Bool}`: matrix of bad channels
...
function detect_bad(
    ...
    amp_t::Float64=400.0,
)::@NamedTuple{bm::Vector{Bool}, be::Vector{Int64}}
    ...
    if :flat in method
        ...
        @inbounds for ep_idx in 1:ep_n
            ...
            [bad_chs[ch_idx] && (bm[ch_idx] = true) for ch_idx in 1:ch_n]
    ...
    if :rmse in method
        ...
        @inbounds for ep_idx in 1:ep_n
            ...
            for ch_idx in 1:ch_n
                if rmse_ch[ch_idx] < HypothesisTests.confint(OneSampleTTest(rmse_ch))[1] ||
    ...
    if :rmsd in method
        ...
        @inbounds for ep_idx in 1:ep_n
            ...
            rmsd_ch = zeros(ch_n)
            for ch_idx in 1:ch_n
                rmsd_ch[ch_idx] = @views Distances.rmsd(
                    obj.data[ch[ch_idx], :, ep_idx], ch_m
                )
            end
            for ch_idx in 1:ch_n
                if rmsd_ch[ch_idx] < HypothesisTests.confint(OneSampleTTest(rmsd_ch))[1] ||
    ...
    if :euclid in method
        ...
        @inbounds for ep_idx in 1:ep_n
            ...
            ed_ch = zeros(ch_n)
            for ch_idx in 1:ch_n
                ed_ch[ch_idx] = @views Distances.euclidean(
                    obj.data[ch[ch_idx], :, ep_idx], ch_m
                )
            end
            for ch_idx in 1:ch_n
                if ed_ch[ch_idx] < HypothesisTests.confint(OneSampleTTest(ed_ch))[1] ||
        ...
        @inbounds for ep_idx in 1:ep_n
            ...
            for ch_idx in 1:ch_n
                ...
                for length_idx in 1:w:(length(obj.data[ch[ch_idx], :, ep_idx]) - w)
                    @views push!(
                        s, median(obj.data[ch[ch_idx], length_idx:(length_idx + w), ep_idx])
                    )
                end
    ...
    if :tkeo in method
        ...
        @inbounds for ep_idx in 1:ep_n
            ...
            for ch_idx in 1:ch_n
                s = @views tkeo(
                    obj.data[ch[ch_idx], :, ep_idx], obj.epoch_time, method=tkeo_method
                )
                z_signal = vec(NeuroAnalyzer.zscore(obj.data[ch[ch_idx], :, ep_idx]))
                z_tkeo = vec(NeuroAnalyzer.zscore(s))
                # scan in 10-sample windows
                w = length(s) ÷ 10
                bad_windows = 0
                for idx in 1:w:length(s)
                    count(
                        abs.(z_signal[idx:(idx + w - 1)]) -
                        abs.(z_tkeo[idx:(idx + w - 1)]) .> cl2z(p),
                    ) > 1 && (bad_windows += 1)
                end
