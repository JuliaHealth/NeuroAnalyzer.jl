...
function detect_bad(obj::NeuroAnalyzer.NEURO; channel::Union{Int64, Vector{Int64}, <:AbstractRange}=get_channel_bytype(obj, type=Symbol(obj.header.recording[:data_type])), method::Vector{Symbol}=[:flat, :rmse, :rmsd, :euclid, :p2p], w::Int64=10, ftol::Float64=0.1, fr::Float64=0.3, p::Float64=0.95, tc::Float64=0.2)
    ...
    if :flat in method
        ...
        @inbounds @simd for ep_idx in 1:ep_n
            ...
            Threads.@threads for ch_idx in 1:ch_n
                s = @views obj.data[channel[ch_idx], :, ep_idx]
                sm = Vector{Float64}()
            ...
            [bad_channels[ch_idx] == true && (bm[ch_idx, ep_idx] = true) for ch_idx in 1:ch_n]
            (bad_channels_score / ch_n) > tc && push!(be, ep_idx)
        end
    end
    ...
    if :rmse in method
        ...
        @inbounds @simd for ep_idx in 1:ep_n
            ch_m = @views vec(median(obj.data[channel, :, ep_idx], dims=1))
            bad_channels_score = 0
            bad_channels = zeros(Bool, ch_n)

            rmse_ch = zeros(ch_n)
            Threads.@threads for ch_idx in 1:ch_n
                rmse_ch[ch_idx] = @views rmse(obj.data[channel[ch_idx], :, ep_idx], ch_m)
            end
            Threads.@threads for ch_idx in 1:ch_n
    ...
    if :rmsd in method
        ...
        @inbounds @simd for ep_idx in 1:ep_n
            ...
            rmsd_ch = zeros(ch_n)
            Threads.@threads for ch_idx in 1:ch_n
                rmsd_ch[ch_idx] = @views Distances.rmsd(obj.data[channel[ch_idx], :, ep_idx], ch_m)
            end
            Threads.@threads for ch_idx in 1:ch_n
    ...
    if :euclid in method
        ...
        @inbounds @simd for ep_idx in 1:ep_n
            ...
            ed_ch = zeros(ch_n)
            Threads.@threads for ch_idx in 1:ch_n
                ed_ch[ch_idx] = @views Distances.euclidean(obj.data[channel[ch_idx], :, ep_idx], ch_m)
            end
            Threads.@threads for ch_idx in 1:ch_n
    ...
    if :var in method
        ...
        s_v = @views var(obj.data[channel, :, :], dims=2)
        # mean variance
        ...
        @inbounds @simd for ep_idx in 1:ep_n
            bad_channels_score = 0
            bad_channels = zeros(Bool, ch_n)
            ch_v = @views vec(var(obj.data[channel, :, ep_idx], dims=2))
            s_mv = vcat(s_mv, ch_v)
            Threads.@threads for ch_idx in 1:ch_n
    ...
    if :p2p in method
        ...
        @inbounds @simd for ep_idx in 1:ep_n
            ...
            Threads.@threads for ch_idx in 1:ch_n
                s = Vector{Float64}()
                for length_idx in 1:w:(length(obj.data[channel[ch_idx], :, ep_idx]) - w)
                    @views push!(s, median(obj.data[channel[ch_idx], length_idx:(length_idx + w), ep_idx]))
                end
                p2p = @views round.(diff(s), digits=-2)
                s_m = @views mean(obj.data[channel[ch_idx], :, ep_idx])
                s_s = @views std(obj.data[channel[ch_idx], :, ep_idx])
            ...
            for ch_idx in 1:ch_n
                bad_channels[ch_idx] == true && (bm[ch_idx, ep_idx] = true)
            end 
            (bad_channels_score / ch_n) > tc && push!(be, ep_idx)
        end
    end
