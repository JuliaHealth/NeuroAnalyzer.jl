...
function mplot_locs(locs::DataFrame; ch::Union{Int64, Vector{Int64}, AbstractRange}=1:DataFrames.nrow(locs), selected::Union{Int64, Vector{Int64}, AbstractRange}=0, ch_labels::Bool=true, head::Bool=true, head_labels::Bool=false, mono::Bool=false, grid::Bool=false, large::Bool=true, cart::Bool=false, plane::Symbol=:xy, transparent::Bool=false, connections::Matrix{<:Real}=[0 0; 0 0], threshold::Real=0, threshold_type::Symbol=:neq, weights::Union{Bool, Vector{<:Real}}=true)::GLMakie.Figure
    ...
    if grid
        ...
        if large
            # prepare plot
            plot_size = head12 ? size(head_shape) : size(head_shape) .+ 50
            p = GLMakie.Figure(size=plot_size)
            ax = GLMakie.Axis(p[1, 1],
                              aspect=1,
                              xlabel="",
            ...
            # prepare plot
            plot_size = head12 ? size(head_shape) : size(head_shape) .+ 7
            p = GLMakie.Figure(size=plot_size)
            ax = GLMakie.Axis(p[1, 1],
                              aspect=1,
                              xlabel="",
                            ...
                            if weights
                                if connections[idx1, idx2] > 0
                                    if mono
                                        ...
                                        Plots.plot!([loc_x[idx1], loc_x[idx2]],
                                                    [loc_y[idx1], loc_y[idx2]],
                                                    linewidth=6 * m_tmp[idx1, idx2],
                                                    alpha=0.25 * m_tmp[idx1, idx2],
                                                    color=:red)
                                ...
                                if mono
                                    Plots.plot!(annotations=(l_pos[1],
                                                             l_pos[2],
                                                             Plots.text(connections[idx1, idx2], pointsize=font_size+2)))
                                else
                                    if connections[idx1, idx2] >= 0
                                        Plots.plot!(annotations=(l_pos[1],
                                                                 l_pos[2],
                                                                 Plots.text(connections[idx1, idx2], pointsize=font_size+2, :red)))
                                    else
                                        Plots.plot!(annotations=(l_pos[1],
                                                                 l_pos[2],
                                                                 Plots.text(connections[idx1, idx2], pointsize=font_size+2, :blue)))
                                    end
                                end
                            end
                        end
                    elseif threshold_type === :l
                        if connections[idx1, idx2] < threshold
                            if weights
                                ...
                                l_pos = _midxy(loc_x[idx1], loc_y[idx1], loc_x[idx2], loc_y[idx2])
                                if mono
                                    Plots.plot!(annotations=(l_pos[1], l_pos[2],
                                                             Plots.text(connections[idx1, idx2], pointsize=font_size+2)))
                                else
                                    if connections[idx1, idx2] >= 0
                                        Plots.plot!(annotations=(l_pos[1], l_pos[2],
    ...
    for idx in eachindex(locs[!, :label])
        if idx in ch
            GLMakie.scatter!((loc_x[idx], loc_y[idx]),
                             color=:lightgrey,
                             #strokecolor=Colors.RGBA(255/255, 255/255, 255/255, 0/255),
                             #markershape=:circle,
    ...
    cmap = GLMakie.resample_cmap(pal, length(selected))
    for idx in eachindex(locs[!, :label])
        if idx in selected
            if mono != true
                GLMakie.scatter!((loc_x[idx], loc_y[idx]),
...
#                                 color=Colors.RGBA(255/255, 255/255, 255/255, 0/255),
                                 markersize=marker_size,
                                 alpha=ma)
                               #markerstrokewidth=0,
                               #markerstrokealpha=0)
...
function mplot_gridlocs(; mono::Bool=false)::GLMakie.Figure
    ...
    p = Plots.plot!([-1, 1], [-1, -1], color=:black, linewidth=0.2)
    p = Plots.plot!([-1, 1], [1, 1], color=:black, linewidth=0.2)

    p = Plots.plot!([-1, -1], [-1, 1], color=:black, linewidth=0.2)
    p = Plots.plot!([1, 1], [-1, 1], color=:black, linewidth=0.2)

    p = Plots.plot!([-1, -0.5], [0.5, 1], color=:black, linewidth=0.5)
    p = Plots.plot!([0.5, 1], [1, 0.5], color=:black, linewidth=0.5)
    p = Plots.plot!([-1, -0.5], [-0.5, -1], color=:black, linewidth=0.5)
    p = Plots.plot!([0.5, 1], [-1, -0.5], color=:black, linewidth=0.5)

    p = Plots.plot!([-0.5, 0.5], [-1, -1], color=:black, linewidth=0.5)
    p = Plots.plot!([-1, 1], [-0.5, -0.5], color=:black, linewidth=0.5)
    p = Plots.plot!([-1, 1], [0, 0], color=:black, linewidth=0.5)
    p = Plots.plot!([-1, 1], [0.5, 0.5], color=:black, linewidth=0.5)
    p = Plots.plot!([-0.5, 0.5], [1, 1], color=:black, linewidth=0.5)

    p = Plots.plot!([-1, -1], [-0.5, 0.5], color=:black, linewidth=0.5)
    p = Plots.plot!([-0.5, -0.5], [-1, 1], color=:black, linewidth=0.5)
    p = Plots.plot!([0, 0], [-1, 1], color=:black, linewidth=0.5)
    p = Plots.plot!([0.5, 0.5], [-1, 1], color=:black, linewidth=0.5)
    p = Plots.plot!([1, 1], [-0.5, 0.5], color=:black, linewidth=0.5)
