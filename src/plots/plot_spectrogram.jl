...
function plot_spectrogram(
    ...
    if !isnothing(threshold)
        hm = GLMakie.heatmap!(
            ax, st, sf, sp_threshold; colorrange=extrema(sp[.!isnan.(sp)]), colormap=pal
        )
    else
...
function plot_spectrogram_topo(
    ...
    for idx in axes(sp, 3)
        ...
        pp = FileIO.load(io)
        GLMakie.scatter!(
            loc_x[idx], loc_y[idx]; marker=pp, markersize=marker_size, markerspace=:pixel
        )
    end
...
function plot_spectrogram(
    ...
    if length(ch) == 1 || type === :topo
        ...
        elseif method === :mt
            ...
            if ep != 0
                ...
                title == "default" &&
                    (title = "Spectrogram (multi-tapered)\n[time window: $t_s1:$t_s2]")
            end
        elseif method === :mw
            _, sp, _, sf, st = NeuroAnalyzer.mwspectrogram(
                signal; fs=fs, ncyc=ncyc, db=false, w=w
            )
            if ep != 0
                title == "default" && (title = "Spectrogram (Morlet wavelet)\n[epoch: $ep]")
            else
                title == "default" &&
                    (title = "Spectrogram (Morlet wavelet)\n[time window: $t_s1:$t_s2]")
            end
        elseif method === :gh
            sp, _, sf, st = NeuroAnalyzer.ghtspectrogram(
                signal; fs=fs, db=false, gw=gw, w=w
            )
            if ep != 0
                title == "default" &&
        ...
        elseif method === :hht
            imf = emd(signal, t)
            sp, _, sf, st = NeuroAnalyzer.hhtspectrogram(
                imf[1:(end - 1), :]; fs=fs, db=false
            )
            if ep != 0
                title == "default" && (title = "Spectrogram (Hilbert-Huang)\n[epoch: $ep]")
            else
                title == "default" &&
                    (title = "Spectrogram (Hilbert-Huang)\n[time window: $t_s1:$t_s2]")
            end
        end
    elseif length(ch) > 1 && type === :normal
        if method === :stft
            sp, sf = psd(
                signal; fs=fs, db=db, method=:stft, nt=nt, wlen=wlen, woverlap=woverlap, w=w
            )
            if ep != 0
                title == "default" &&
                    (title = "Spectrogram (short-time Fourier)\n[epoch: $ep]")
        ...
        elseif method === :mt
            sp, sf = psd(
                signal; fs=fs, db=db, method=:mt, nt=nt, wlen=wlen, woverlap=woverlap, w=w
            )
            if ep != 0
                title == "default" && (title = "Spectrogram (multi-tapered)\n[epoch: $ep]")
            else
        ...
        elseif method === :hht
            imf = emd(signal[1, :], t)
            sp_tmp, _, sf, st = NeuroAnalyzer.hhtspectrogram(
                imf[1:(end - 1), :]; fs=fs, db=db
            )
            sp = zeros(size(signal, 1), length(sp_tmp))
            sp[1, :] = sp_tmp
            for idx in axes(signal, 1)[(begin + 1):end]
                imf = emd(signal[idx, :], t)
                sp[idx, :], _, _, _ = NeuroAnalyzer.hhtspectrogram(
                    imf[1:(end - 1), :]; fs=fs, db=db
                )
            end
            if ep != 0
    ...
    if length(ch) == 1 && markers && _has_markers(obj)
        ...
        if gui
            GLMakie.vlines!(
                p[2, 1], markers_pos; linestyle=:dash, linewidth=1, color=:black
            )
        end
        for idx in eachindex(markers_pos)
            if _in(markers_pos[idx], (obj.time_pts[1], obj.time_pts[end]))
                GLMakie.vlines!(
                    p[1, 1], markers_pos[idx]; linestyle=:dash, linewidth=1, color=:black
                )
                if length(ch) > 1
                    GLMakie.textlabel!(
                        p[1, 1],
