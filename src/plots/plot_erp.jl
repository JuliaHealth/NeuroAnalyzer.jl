...
function plot_erp_topo(
    ...
    for idx in axes(s, 1)
        ...
        push!(pp_vec, pp)
        pp_full = plot_erp(
            t, s[idx, :]; xlabel=xlabel, ylabel=ylabel, title=title, rt=rt, yrev=yrev
        )
...
function plot_erp_stack(
    ...
    ax = GLMakie.Axis(
        ...
        xautolimitmargin=(0, 0),
        yautolimitmargin=(0, 0),
        yticks=if size(s, 1) <= 30
            (1:length(clabels), clabels)
        else
            (5:5:length(clabels), clabels[5:5:end])
        end,
...
function plot_erp(
    ...
    if length(ch) == 1
        if type === :stack
            ...
            p = plot_erp(
                t, s; xlabel=xl, ylabel=yl, title=tt, rt=rt, yrev=yrev, zl=zl, mono=mono
            )
        end
    elseif type === :normal
