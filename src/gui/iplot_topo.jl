export iplot_topo
export iplot_topo_ep
export iplot_topo_cont
...
"""
function iplot_topo(obj::NeuroAnalyzer.NEURO; ch::Union{Int64, Vector{Int64}, <:AbstractRange}=get_channel_bytype(obj, type=datatype(obj)), mono::Bool=true, zoom::Real=5, snap::Bool=true)
...
- `snap::Bool=true`: snap region markers to grid at 0.0, 0.25, 0.5 and 0.75 time points
"""
function iplot_topo_cont(obj::NeuroAnalyzer.NEURO; ch::Union{Int64, Vector{Int64}, <:AbstractRange}=get_channel_bytype(obj, type=datatype(obj)), mono::Bool=true, zoom::Real=5, snap::Bool=true)
...
function iplot_topo_ep(obj::NeuroAnalyzer.NEURO; ch::Union{Int64, Vector{Int64}, <:AbstractRange}=get_channel_bytype(obj, type=datatype(obj)), mono::Bool=true)
    ...
    @assert nepochs(obj) > 1 "iplot_topo_cont() should be used for continuous object."
    _check_channels(obj, ch)

    p = NeuroAnalyzer.plot(obj, ch=ch, ep=1, mono=mono, title="")
    win = GtkWindow("NeuroAnalyzer: iplot_topo_ep()", 1200, 800)
    win_view = GtkScrolledWindow()
    set_gtk_property!(win_view, :min_content_width, 1200)
    set_gtk_property!(win_view, :min_content_height, 800)
    set_gtk_property!(win, :border_width, 20)
