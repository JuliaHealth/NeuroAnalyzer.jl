    ...
    function _activate(app)
        ...
        @guarded draw(can) do widget
            ...
            elseif length(get_channel(obj, ch=ch)) < 2 && type === :butterfly
                warn_dialog(
                    _nill, "For butterfly plot, the signal must contain ≥ 2 channels.", win
                )
                no_error = false
            elseif length(get_channel(obj, ch=ch)) < 2 && type === :mean
                warn_dialog(
                    _nill, "For mean plot, the signal must contain ≥ 2 channels.", win
                )
                no_error = false
            elseif length(get_channel(obj, ch=ch)) < 2 && type === :w3d
                warn_dialog(
                    _nill, "For w3d plot, the signal must contain ≥ 2 channels.", win
                )
                no_error = false
            elseif length(get_channel(obj, ch=ch)) < 2 && type === :s3d
                warn_dialog(
                    _nill, "For s3d plot, the signal must contain ≥ 2 channels.", win
                )
                no_error = false
            ...
            ) > 1 && (type in [:butterfly, :mean, :w3d, :s3d, :topo] || ch == "all")
                warn_dialog(
                    _nill,
