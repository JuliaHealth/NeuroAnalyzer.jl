...
function ispectrogram(obj::NeuroAnalyzer.NEURO; ch::String, zoom::Real = 10)::Nothing
    !(nepochs(obj) == 1) &&
        throw(ArgumentError("For epoched object ispectrogram_ep() must be used."))
    ...
    function _activate(app)
        ...
        cb_smooth.tooltip_text = "Smooth the spectrogram image"
        cb_smooth.active = true

        combo_method = GtkComboBoxText()
        spectrogram_methods = [
            "short-time Fourier transform", "multi-taper", "Morlet wavelet",
        ...
        combo_method.tooltip_text = "Spectrogram method"

        entry_wt = GtkEntry()
        entry_wt.text = "Morlet(2π), β=2"
        entry_wt.tooltip_text = "Continuous wavelet formula"

        entry_nt = GtkSpinButton(1, 128, 1)
        entry_nt.value = 8
        entry_nt.tooltip_text = "Number of Slepian tapers"
        ...
        entry_n.value = 3
        entry_n.tooltip_text = "Gaussian smoothing filter kernel size"
        ...
        @guarded draw(can) do widget
            ...
            hw = cb_hw.active
            smooth = cb_smooth.active
            n = Int64(entry_n.value)
            ...
            try
                wt = eval(
                    Meta.parse(
                        "wavelet(" * get_gtk_property(entry_wt, :text, String) * ")",
                    ),
                )
            catch
            end
            ...
            if frq1 == frq2
                ...
                no_error = false
            elseif wt === nothing
                warn_dialog(_nill, "Incorrect wavelet formula.", win)
                no_error = false
            elseif frq1 > frq2
        ...
        signal_connect(cb_smooth, "toggled") do widget
            return draw(can)
        end
...
function ispectrogram_ep(obj::NeuroAnalyzer.NEURO; ch::String)::Nothing
    !(nepochs(obj) > 1) &&
        throw(ArgumentError("For continuous object ispectrogram() must be used."))
    ...
    function _activate(app)
        ...
        bt_start = GtkButton("⇤")
        bt_start.tooltip_text = "Go to the first epoch start"
        bt_end = GtkButton("⇥")
