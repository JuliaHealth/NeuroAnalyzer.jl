...
- `seg::Tuple{Real, Real}=(0, 0)`: segment (from, to) in seconds to display
"""
function itopo_cont(obj::NeuroAnalyzer.NEURO; ch::Union{String, Vector{String}}, seg::Tuple{Real, Real}=(0, 0))
...
function itopo_ep(obj::NeuroAnalyzer.NEURO; ch::Union{String, Vector{String}})
    ...
    g_opts[1, 2] = lab_t
    g_opts[1, 3] = lab_type
    g_opts[1, 4] = lab_amethod
    g_opts[1, 5] = lab_nmethod
    g_opts[1, 6] = lab_cb
    g_opts[1, 7] = lab_cb_draw
    g_opts[1, 8] = lab_cart
    g_opts[1, 9] = lab_large
    g_opts[1, 10] = lab_elec
    g_opts[1, 11] = lab_contour
    g_opts[2, 1] = entry_epoch
    g_opts[2, 2] = entry_title
    g_opts[2, 3] = combo_imethod
    g_opts[2, 4] = combo_amethod
    g_opts[2, 5] = combo_nmethod
    g_opts[2, 6] = entry_cblab
    g_opts[2, 7] = cb_cb
    g_opts[2, 8] = cb_cart
    g_opts[2, 9] = cb_large
    g_opts[2, 10] = cb_elec
    g_opts[2, 11] = cb_contour
    g_opts[1:2, 12] = bt_refresh
    g_opts[1, 13] = bt_help
    g_opts[2, 13] = bt_close
