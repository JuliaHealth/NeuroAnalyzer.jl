...
function cpsd(
    ...
    if method === :mt
        ...
        s = hcat(s1 .* w, s2 .* w)'
        pxy = DSP.mt_cross_power_spectra(
            s;
            fs=fs,
            demean=demean,
            nfft=nextpow(2, n_samples + 1),
            nw=((nt + 1) ÷ 2),
            ntapers=nt,
        )
        f = DSP.freq(pxy)
    ...
    elseif method === :stft
        ...
        for idx in axes(chunks_idx, 1)
            if demean
                s1_tmp = @views detrend(
                    s1[chunks_idx[idx, 1]:chunks_idx[idx, 2]], type=:mean
                )
                s2_tmp = @views detrend(
                    s2[chunks_idx[idx, 1]:chunks_idx[idx, 2]], type=:mean
                )
            else
...
function cpsd(
    ...
    @inbounds for ep_idx in 1:ep_n
        Threads.@threads for ch_idx in 1:ch_n
            pxy[ch_idx, :, ep_idx], _ = @views cpsd(
                s1[ch_idx, :, ep_idx],
                s2[ch_idx, :, ep_idx],
                method=method,
                fs=fs,
