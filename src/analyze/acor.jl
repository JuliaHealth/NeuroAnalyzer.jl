...
function acor(
    ...
    @inbounds for ep_idx in 1:ep_n
        ac[1, :, ep_idx] = @views acor(
            s[:, ep_idx], l=l, demean=demean, biased=biased, method=method
        )
    end
...
function acor(
    ...
    if datatype(obj) == "erp"
        ac = @views acor(
            obj.data[ch, :, 2:end], l=l, demean=demean, biased=biased, method=method
        )
        ac = cat(mean(ac; dims=3), ac; dims=3)
    else
        ac = @views acor(
            obj.data[ch, :, :], l=l, demean=demean, biased=biased, method=method
        )
    end
