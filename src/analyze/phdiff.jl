...
"""
function phdiff(s1::Vector{<:Real}, s2::Vector{<:Real}; pad::Int64=0, h::Bool=false)::Vector{Float64}
...
- `phd::Array{Float64, 3}`
"""
function phdiff(s::Array{<:Real, 3}; ch::Union{Int64, Vector{Int64}}=_c(size(s, 1)), avg::Symbol=:phase, pad::Int64=0, h::Bool=false)::Array{Float64, 3}
        ...
        Threads.@threads for ch_idx in 1:ch_n
            if avg === :phase
                ...
                for ref_idx in eachindex(ref_channels)
                    if h
                        _, _, _, ph = hspectrum(s[ref_channels[ref_idx], :, ep_idx], pad=pad)
                    else
                        _, _, _, ph = spectrum(s[ref_channels[ref_idx], :, ep_idx], pad=pad)
                    end
                    ph_ref[ref_idx, :] = ph
                end
                ...
                if h
                    _, _, _, ph = hspectrum(s[ch[ch_idx], :, ep_idx], pad=pad)
                else
                    _, _, _, ph = spectrum(s[ch[ch_idx], :, ep_idx], pad=pad)
                end
                ...
                phd[ch_idx, :, ep_idx] = phdiff(s[ch[ch_idx], :, ep_idx], signal_m)
            end
        end
    end
...
function phdiff(obj::NeuroAnalyzer.NEURO; ch::Union{String, Vector{String}}, avg::Symbol=:phase, pad::Int64=0, h::Bool=false)::Array{Float64, 3}
    ...
    phd = phdiff(obj.data[ch, :, :], avg=avg, pad=pad, h=h)
