...
"""
function cph(
    s1::AbstractVector, s2::AbstractVector; fs::Int64
)::@NamedTuple{ph::Vector{Float64}, f::Vector{Float64}}
    ...
    @inbounds for ep_idx in 1:ep_n
        Threads.@threads for ch_idx1 in 1:ch_n
            for ch_idx2 in 1:ch_idx1
                ph[ch_idx1, ch_idx2, :, ep_idx], _ = @views cph(
                    s[ch_idx1, :, ep_idx], s[ch_idx2, :, ep_idx], fs=fs
                )
            end
        ...
        Threads.@threads for ep_idx in 1:ep_n
            for ch_idx1 in 1:(ch_n - 1)
                for ch_idx2 in (ch_idx1 + 1):ch_n
                    ph[ch_idx1, ch_idx2, cph_idx, ep_idx] = @views ph[
                        ch_idx2, ch_idx1, cph_idx, ep_idx
                    ]
                end
            end
        end
...
"""
function cph(
    s1::AbstractArray, s2::AbstractArray; fs::Int64
    ...
    @inbounds for ep_idx in 1:ep_n
        Threads.@threads for ch_idx in 1:ch_n
            ph[ch_idx, :, ep_idx], _ = @views cph(
                s1[ch_idx, :, ep_idx], s2[ch_idx, :, ep_idx], fs=fs
            )
        end
    end
