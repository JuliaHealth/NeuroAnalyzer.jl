...
"""
function cph(s1::Vector{<:Real}, s2::Vector{<:Real}; fs::Int64)::@NamedTuple{ph::Vector{Float64}, f::Vector{Float64}}
...
- `f::Vector{Float64}`: cross-power spectrum frequencies
"""
function cph(s::Array{<:Real, 3}; fs::Int64)::@NamedTuple{ph::Array{Float64, 4}, f::Vector{Float64}}

    ch_n = size(s, 1)
    ep_n = size(s, 3)

    _, f = cph(s[1, :, 1], s[1, :, 1], fs=fs)
    ...
    @inbounds for ep_idx in 1:ep_n
        Threads.@threads for ch_idx1 in 1:ch_n
           for ch_idx2 in 1:ch_idx1
                ph[ch_idx1, ch_idx2, :, ep_idx], _ = cph(s[ch_idx1, :, ep_idx], s[ch_idx2, :, ep_idx], fs=fs)
            end
        ...
        Threads.@threads for ep_idx in 1:ep_n
            for ch_idx1 in 1:(ch_n - 1)
                for ch_idx2 in (ch_idx1 + 1):ch_n
                    ph[ch_idx1, ch_idx2, cph_idx, ep_idx] = ph[ch_idx2, ch_idx1, cph_idx, ep_idx]
                end
            end
        end
    end
...
- `f::Vector{Float64}`: cross-power spectrum frequencies
"""
function cph(s1::Array{<:Real, 3}, s2::Array{<:Real, 3}; fs::Int64)::@NamedTuple{ph::Array{Float64, 3}, f::Vector{Float64}}
    ...
    _, f = cph(s1[1, :, 1], s2[1, :, 1], fs=fs)
    ph = zeros(ch_n, length(f), ep_n)

    @inbounds for ep_idx in 1:ep_n
        Threads.@threads for ch_idx in 1:ch_n
            ph[ch_idx, :, ep_idx], _ = cph(s1[ch_idx, :, ep_idx], s2[ch_idx, :, ep_idx], fs=fs)
        end
    end
...
function cph(obj1::NeuroAnalyzer.NEURO, obj2::NeuroAnalyzer.NEURO; ch1::Union{String, Vector{String}}, ch2::Union{String, Vector{String}}, ep1::Union{Int64, Vector{Int64}, AbstractRange}=_c(nepochs(obj1)), ep2::Union{Int64, Vector{Int64}, AbstractRange}=_c(nepochs(obj2)))::@NamedTuple{ph::Array{Float64, 3}, f::Vector{Float64}}
    ...
    ph, f = cph(obj1.data[ch1, :, ep1], obj2.data[ch2, :, ep2], fs=sr(obj1))
