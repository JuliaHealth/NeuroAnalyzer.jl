...
"""
    band_mpower(s; fs, f, mt, st, nt, wlen, woverlap, w)
...
"""
function band_mpower(s::AbstractVector; fs::Int64, f::Tuple{Real, Real}, mt::Bool=false, st::Bool=false, nt::Int64=8, wlen::Int64=fs, woverlap::Int64=round(Int64, wlen * 0.97), w::Bool=true)
...
"""
function band_mpower(s::AbstractArray; fs::Int64, f::Tuple{Real, Real}, mt::Bool=false, st::Bool=false, nt::Int64=8, wlen::Int64=fs, woverlap::Int64=round(Int64, wlen * 0.97), w::Bool=true)
    ...
    @inbounds @simd for ep_idx in 1:ep_n
        Threads.@threads for ch_idx in 1:ch_n
            mbp[ch_idx, ep_idx], maxfrq[ch_idx, ep_idx], maxbp[ch_idx, ep_idx] = @views band_mpower(s[ch_idx, :, ep_idx], fs=fs, f=f, mt=mt, st=st, nt=nt, wlen=wlen, woverlap=woverlap, w=w)
        end
    end
...
"""
    band_mpower(obj; ch, f, mt, st, nt, wlen, woverlap, w)
...
- `maxbp::Matrix{Float64}`: power at maximum band frequency per channel per epoch
"""
function band_mpower(obj::NeuroAnalyzer.NEURO; ch::Union{Int64, Vector{Int64}, <:AbstractRange}=signal_channels(obj), f::Tuple{Real, Real}, mt::Bool=false, st::Bool=false, nt::Int64=8, wlen::Int64=sr(obj), woverlap::Int64=round(Int64, wlen * 0.97), w::Bool=true)

    _check_channels(obj, ch)
    
    mbp, maxfrq, maxbp = @views band_mpower(obj.data[ch, :, :], fs=sr(obj), f=f, mt=mt, st=st, nt=nt, wlen=wlen, woverlap=woverlap, w=w)
