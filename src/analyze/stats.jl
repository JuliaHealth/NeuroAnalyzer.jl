...
function epoch_stats(
    ...
    @inbounds for ep_idx in 1:ep_n
        ...
        e_max_dif[ep_idx] = @views maximum(obj.data[:, :, ep_idx]) -
            minimum(obj.data[:, :, ep_idx])
        e_dev_mean[ep_idx] = @views abs(mean(obj.data[:, :, ep_idx])) -
            mean(obj.data[:, :, ep_idx])
    end
    ...
    return (
        ...
        e_kurt=e_kurt,
        e_skew=e_skew,
        e_mean_diff=e_mean_diff,
        e_median_diff=e_median_diff,
        e_max_dif=e_max_dif,
        e_dev_mean=e_dev_mean,
    )
end
...
function channel_stats(
    ...
    return (
        ...
        c_median_diff=c_median_diff,
        c_max_dif=c_max_dif,
        c_dev_mean=c_dev_mean,
    )
end
