...
function pacor(
    ...
    @inbounds for ep_idx in 1:ep_n
        pac[1, :, ep_idx] = @views reshape(
            pacor(s[:, ep_idx], l=l, demean=demean, method=method), 1, :, ep_n
        )
    end
...
function pacor(
    ...
    @inbounds for ep_idx in 1:ep_n
        Threads.@threads for ch_idx in 1:ch_n
            pac[ch_idx, :, ep_idx] = @views pacor(
                s[ch_idx, :, ep_idx], l=l, demean=demean, method=method
            )
        end
    end
