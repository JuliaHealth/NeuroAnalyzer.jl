...
function hhtspectrogram(
    ...
    t = round.(linspace(t[1], t[end - 1], size(p, 2)), digits = 4)
    ...
    @inbounds for idx1 in axes(s, 1)
        imfs = emd(s[idx1, :], t)[1:(end - 1), :]
        imfs_n = size(imfs, 1)
        # last row is the residual
        for idx2 in 1:(imfs_n - 1)
            # Hilbert transform -> powers
            push!(imf_p, htransform(@view(imfs[idx2, :])).p)
            # instantaneous frequencies cycles per sample -> Hz
            push!(imf_fi, frqinst(@view(imfs[idx2, :])) .* fs)
        end
    end
