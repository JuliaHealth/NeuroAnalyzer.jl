...
function remove_powerline(
    ...
    # validate
    nepochs(obj) == 1 ||
        throw(ArgumentError("remove_powerline() requires a continuous (1-epoch) object."))
    pl_frq >= 0 || throw(ArgumentError("pl_frq must be ≥ 0."))
    pl_frq <= sr(obj) / 2 ||
        throw(ArgumentError("pl_frq must be ≤ $(sr(obj) / 2) Hz (Nyquist)."))
    ...
    if method === :iir
        ...
        for (loop_idx, ch_idx) in enumerate(ch)
            ...
            if isempty(pks_frq)
                pr_tmp, d_tmp = pr, d
                peaks, _ =
                    findpeaks1d(p_above; prominence = pr_tmp, distance = vsearch(d_tmp, f))
                while isempty(peaks)
                    pr_tmp -= 0.1
                    d_tmp -= 0.1
                    peaks, _ = findpeaks1d(
...
function detect_powerline(obj::NeuroAnalyzer.NEURO)::Array{Float64, 2}
    ...
    progbar =
        Progress(ch_n * ep_n; dt = 1, barlen = 20, color = :white, enabled = progress_bar)

    # calculate over channel and epochs
    @inbounds Threads.@threads :static for idx in CartesianIndices((ch_n, ep_n))
        ch_idx, ep_idx = idx[1], idx[2]
        noise_frq[ch_idx, ep_idx] =
            detect_powerline(@view(obj.data[ch_idx, :, ep_idx]); fs = sr(obj))
