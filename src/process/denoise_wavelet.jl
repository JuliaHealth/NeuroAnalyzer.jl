...
function denoise_cwd(
    ...
    obj_new.data[ch, :, :] = @views denoise_cwd(
        obj.data[ch, :, :], fs=sr(obj), wt=wt, nf=nf, w=w, type=type
    )
...
function denoise_dwd(
    ...
    ch = get_channel(obj; ch=ch)
    obj_new = deepcopy(obj)
    obj_new.data[ch, :, :] = @views denoise_dwd(
        obj.data[ch, :, :], wt=wt, l=l, dnt=dnt, smooth=smooth
    )
    push!(
        obj_new.history, "denoise_dwd(OBJ, ch=$ch, wt=$wt, l=$l, dnt=$dnt, smooth=$smooth))"
    )
