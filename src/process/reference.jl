    ...
    @inbounds for ep_idx in 1:ep_n
        Threads.@threads :dynamic for ch_idx in ch
            if length(ch) == 1
                ...
                ref_ch =
                    med ? (@views vec(median(s[ch, :, ep_idx], dims = 1))) :
...
function reference_a(obj::NeuroAnalyzer.NEURO; type::Symbol = :l, med::Bool = false)::NeuroAnalyzer.NEURO
    ...
    elseif type === :i
        c_picks = get_channel(obj, ch = channel_pick(obj, pick = :central))
        @inbounds for ep_idx in 1:ep_n
            if !med
                ref_ch = @views vec(mean(vcat(a1[:, :, ep_idx], a2[:, :, ep_idx]), dims = 1))
            else
        ...
        l_picks = get_channel(obj, ch = channel_pick(obj, pick = :left))
        @inbounds for ep_idx in 1:ep_n
        ...
        r_picks = get_channel(obj, ch = channel_pick(obj, pick = :right))
        @inbounds for ep_idx in 1:ep_n
    ...
    @inbounds for ep_idx in 1:ep_n
        Threads.@threads :dynamic for ch_idx in 1:ch_n
            ...
            if !weighted
                ...
                for w_idx in 1:nn
                    w[w_idx] = euclidean(
                        [loc_x[ch_idx], loc_y[ch_idx]], [loc_x[nn_idx[ch_idx, w_idx]], loc_y[nn_idx[ch_idx, w_idx]]]
                    )
                end
                w = 1 .- w
                if !med
...
function reference_custom(
    ...
    obj_new.header.recording[:prefiltering] = vcat(
        repeat([obj.header.recording[:prefiltering][1]], length(ref_list)), obj_new.header.recording[:prefiltering]
    )
    obj_new.header.recording[:transducers] = vcat(
        repeat([obj.header.recording[:transducers][1]], length(ref_list)), obj_new.header.recording[:transducers]
    )
    obj_new.header.recording[:gain] = vcat(
        repeat([obj.header.recording[:gain][1]], length(ref_list)), obj_new.header.recording[:gain]
    )
