...
load_locs!(
    e10; file_name=joinpath(NeuroAnalyzer.PATH, "locs", "standard-10-20-cap19-elmiko.ced")
)
...
NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:chebyshev1, ftype=:lp, cutoff=45.0, order=8, rs=45
)
@time NeuroAnalyzer.filter(
    e10, ch="all", fprototype=:butterworth, ftype=:lp, cutoff=45.0, order=8, rs=45
)
print(rpad("Filter: IIR HP", 36))
NeuroAnalyzer.filter(e10; ch="all", fprototype=:butterworth, ftype=:hp, cutoff=0.1, order=8)
@time NeuroAnalyzer.filter(
    e10, ch="all", fprototype=:butterworth, ftype=:hp, cutoff=0.1, order=8
)
...
NeuroAnalyzer.filter(
    e10; ch="all", fprototype=:remez, ftype=:lp, order=128, cutoff=20, bw=0.5
)
@time NeuroAnalyzer.filter(
    e10, ch="all", fprototype=:remez, ftype=:lp, order=128, cutoff=20, bw=0.5
)
