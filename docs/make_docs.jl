...
@info "Clone NA-docs" run(`git clone https://codeberg.org/AdamWysokinski/NeuroAnalyzer-docs NA-docs`);

@info "Clone NA-stable" run(`git clone -b stable https://codeberg.org/AdamWysokinski/NeuroAnalyzer.jl NA-stable`);
@info "Go to NA-stable/docs/ folder" cd("NA-stable/docs");
@info "Delete build/ folder" run(`rm -rf build`);
@info "Delete index.md file" run(`rm -f src/index.md`);
@info "Create new index.md file (1)" run(`cat header-stable.md > src/index.md`);
@info "Create new index.md file (2)" run(`./template.sh >> src/index.md`);

@info "Generate Markdown documentation"
makedocs(sitename="NeuroAnalyzer.jl", format=Markdown())
@info "Create Documentation-stable.md file" run(`cp build/index.md ../../NA-docs/Documentation-stable.md`);
@info "Delete build/ folder" run(`rm -rf build`);
...
@info "Go to NA-devel/docs/ folder" cd("NA-devel/docs");
@info "Delete build/ folder" run(`rm -rf build`);
@info "Delete index.md file" run(`rm -f src/index.md`);
@info "Create new index.md file (1)" run(`cat header-devel.md > src/index.md`);
@info "Create new index.md file (2)" run(`./template.sh >> src/index.md`);

@info "Generate Markdown documentation"
makedocs(sitename="NeuroAnalyzer.jl", format=Markdown())
@info "Create Documentation-devel.md file" run(`cp build/index.md ../../NA-docs/Documentation-devel.md`);
@info "Delete build/ folder" run(`rm -rf build`);
